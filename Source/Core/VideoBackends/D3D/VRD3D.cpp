...
void VR_RenderToEyebuffer(int eye, int hmd_number)
{
  ...
  if (g_has_openvr && (hmd_number == 0 || !g_has_rift)) // If OpenVR is primary or Rift is not present
  {
    ...
    {
      AbstractTexture* abstract_eye_texture = g_framebuffer_manager->GetVREyeTexture(eye);
      ...
      {
        ERROR_LOG_FMT(VR, "OpenVR: Eye texture %d is null in FramebufferManager.", eye);
      }
    }
    else
    {
      ERROR_LOG_FMT(VR, "OpenVR: g_framebuffer_manager is null in VR_RenderToEyebuffer.");
    }
  }
#endif
}
...
void VR_PresentHMDFrame()
{
  ...
  {
    AbstractTexture* abs_left_eye_tex = g_framebuffer_manager->GetVREyeTexture(0);
    AbstractTexture* abs_right_eye_tex = g_framebuffer_manager->GetVREyeTexture(1);
    ...
    if (abs_left_eye_tex && abs_right_eye_tex)
    {
      ...
      {
        ERROR_LOG_FMT(VR, "OpenVR: Failed to get D3D textures for submission.");
      }
    }
    else
    {
      ERROR_LOG_FMT(VR, "OpenVR: Eye textures are null in FramebufferManager for submission.");
    }
        ...
        g_ActiveConfig.iMirrorPlayer != VR_PLAYER_NONE)
    {
      ...
      AbstractTexture* mirror_source_tex_abs = g_framebuffer_manager->GetVREyeTexture(eye);
      ...
      if (g_ActiveConfig.iMirrorStyle >= VR_MIRROR_WARPED)
      {
        ...
        D3D::context->RSSetViewports(1, &vp);
        AbstractTexture* mirror_source_tex_abs_right = g_framebuffer_manager->GetVREyeTexture(1);
