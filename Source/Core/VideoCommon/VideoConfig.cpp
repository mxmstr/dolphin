...
void CheckForConfigChanges()
{
  ...
  if (g_ActiveConfig.stereo_mode == StereoMode::OpenVR)
  {
    if (!Core::g_vr_openvr_instance) // If no instance exists at all
    {
      ...
      {
        INFO_LOG_FMT(VR_CORE, "VideoConfig: VR is enabled and no instance exists. Attempting first VR initialization for this session.");
        ...
        {
          ERROR_LOG_FMT(VR_CORE, "Failed to initialize Core::g_vr_openvr_instance. VR will not be available this session unless toggled.");
    ...
    else if (Core::g_vr_openvr_instance && !Core::g_vr_openvr_instance->IsInitialized())
    {
      ...
      if (!s_vr_init_attempted_this_session)
      {
        INFO_LOG_FMT(VR_CORE, "VideoConfig: Found uninitialized VR instance. Attempting initialization for this session.");
        s_vr_init_attempted_this_session = true;
        ...
        if (Core::g_vr_openvr_instance->Init()) // Defaults to VRApplication_Scene
        {
          INFO_LOG_FMT(VR_CORE, "Core::g_vr_openvr_instance re-initialized successfully.");
        }
        else
        {
          ERROR_LOG_FMT(VR_CORE, "Failed to re-initialize Core::g_vr_openvr_instance. VR will not be available this session unless toggled.");
          Core::g_vr_openvr_instance.reset(); // Failed, so reset the pointer. Flag remains true.
        }
      }
  ...
  else // StereoMode is NOT OpenVR
  {
    ...
    {
      INFO_LOG_FMT(VR_CORE, "VideoConfig: VR is disabled. Shutting down existing VR instance.");
