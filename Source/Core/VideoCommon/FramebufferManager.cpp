...
std::tuple<u32, u32> FramebufferManager::CalculateTargetSize()
{
  ...
  if (g_ActiveConfig.bEnableVR && g_ActiveConfig.stereo_mode == StereoMode::OpenVR)
  {
    ...
    INFO_LOG_FMT(VIDEO, "[VR DEBUG] Creating EFB Framebuffer with dimensions: {}x{}", width, height); // <-- ADD THIS
    return std::make_tuple(width, height);
  }
                                  ...
                                  bool alpha_enable, bool z_enable, u32 color, u32 z)
{
  ...
  // --- START OF NEW VR OVERRIDE ---
  if (g_ActiveConfig.bEnableVR)
  {
    // In VR, the game might request a 4:3 clear, but we MUST clear the entire
    // wide-aspect EFB to prevent artifacts in the periphery. This also ensures
    // the render pass is started with a full-sized renderArea.
    target_rc = m_efb_framebuffer->GetRect();
  }
  else
  {
    // Native -> EFB coordinates
    target_rc = ConvertEFBRectangle(rc);
    target_rc = g_gfx->ConvertFramebufferRectangle(target_rc, m_efb_framebuffer.get());
    target_rc.ClampUL(0, 0, m_efb_framebuffer->GetWidth(), m_efb_framebuffer->GetWidth());
  }
  // --- END OF NEW VR OVERRIDE ---
