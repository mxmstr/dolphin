...
DOLPHIN_FORCE_INLINE u32 Run(const u8* data, u32 available, T& callback)
{
      ...
      skipped_opcode_replay_count >= static_cast<int>(g_ActiveConfig.iExtraVideoLoopsDivider))
  {
    ...
    DataReader current_stream_reader((u8*)data, (u8*)(data + available));
...
public:
  ...
  OPCODE_CALLBACK(void OnUnknown(u8 opcode, const u8* data))
  {
    ...
    else
    {
      ...
      ERROR_LOG_FMT(VIDEO, "FIFO: Unknown Opcode(0x{} @ {}, preprocessing = {}, in_dl = {})",
...
{
  using CallbackT = RunCallback<true>;
  auto callback = CallbackT{};
  // Pass 'is_preprocess' to Run
  u32 size_processed = Run<true>(src.GetPointer(), static_cast<u32>(src.size()), callback, false);
