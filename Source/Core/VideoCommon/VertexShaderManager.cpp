...
void VertexShaderManager::TransformToClipSpace(const float* data, float* out, u32 MtxIdx)
{
  ...
  // Corrected multiplication assuming proj_matrix is ROW-MAJOR
  // and t is a column vector (tx, ty, tz, 1.0)
  // Let tw = 1.0f (w-component for position vector t before projection)
  // The world_matrix multiplication already includes the translation, so t is (x', y', z')
  // and we are transforming (x', y', z', 1.0) by proj_matrix.
  const float tw = 1.0f;
  out[0] = t[0]*proj_matrix[0] + t[1]*proj_matrix[1] + t[2]*proj_matrix[2]  + tw*proj_matrix[3];
  out[1] = t[0]*proj_matrix[4] + t[1]*proj_matrix[5] + t[2]*proj_matrix[6]  + tw*proj_matrix[7];
  out[2] = t[0]*proj_matrix[8] + t[1]*proj_matrix[9] + t[2]*proj_matrix[10] + tw*proj_matrix[11];
  out[3] = t[0]*proj_matrix[12] + t[1]*proj_matrix[13] + t[2]*proj_matrix[14] + tw*proj_matrix[15];
}
